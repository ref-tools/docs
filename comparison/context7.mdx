...
| --- | --- | --- |
| Approach | Iterative search + read with sessions | Simple batch retrieval |
| Token Usage | 50-70% savings on average, up to 95% | Baseline 10K tokens per query |
| Tools | `ref_search_documentation`, `ref_read_url` | `resolve-library-id`, `get-library-docs` |
| Advanced MCP features | Stateful search session, pre-built prompts | None |
| Scrape any URL | Yes | No |
