...
Use collapse blocks when a section is useful but long, especially file lists. The plan editor shows the summary by default, and readers can click the chevron to expand the full markdown body.

```md
:::collapse h2
Files — 12 files, 4 dirs
...
`:::collapse h1`, `h2`, or `h3` controls the title weight. Lines before the bare `:::` are the collapsed summary; lines after it and before `:::end` are the expanded body. Agents use collapse blocks automatically for long file lists, usually more than about eight bullets.
