...
| --- | --- | --- |
| Best for | Durable documentation that should live with code | Live planning and coordination before the diff |
| Review timing | Usually reviewed in the same PR as the code change | Reviewed before implementation starts |
| Collaboration model | Static artifact plus PR comments | Comments, review mode, agent threads, and progress updates |
...
- Collaboration happens through commits and PR comments rather than a dedicated planning review flow
...
Ref puts the plan before the diff. The team can discuss scope, architecture, risks, and task breakdown while the work is still cheap to reshape.
...
2. **Active collaboration** - Comments, review mode, parallel agent threads, and progress reporting all happen on the plan itself.
3. **Cross-repo scope** - A plan can cover multiple repos, a new repo that does not exist yet, or a product decision that has not been mapped to code.
4. **Agent-native execution** - Agents launch from the plan, read it as context, ask questions there, and report PRs back to the same workspace.
...
**A live workspace, not only an artifact:** Ref keeps comments, review state, agent threads, progress updates, and PR links together while the work is happening.
