...
People often look to repo markdown to manage engineering specs because it is close to the code and fits existing Git workflows. The challenge is that markdown files were not built for cross-functional spec review, rich planning, or the repeated plan -> do -> plan cycle that AI-assisted engineering creates. Ref is an alternative built specifically for teams that are doing more upfront planning, spec review, and agent orchestration.
