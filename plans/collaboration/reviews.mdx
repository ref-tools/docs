...
A review an agent requests always comes back to you — the account whose API key the session uses — so no reviewer address is involved. To have someone else review the plan, request the review in the app and enter their email address.

Both `request_review` and `AwaitReview` are available only in MCP sessions you connect yourself with your own API key. Agents that Ref launches and runs for you do not get either one: their work is reviewed in the app, on the plan they were launched on.
