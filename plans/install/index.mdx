...
- `Rename` — Change the plan's title
...
- `AwaitReview` — Wait for the plan's review verdict and return it with the reviewer's comments. Each call holds for up to 50 seconds and returns even if nothing has landed yet, so the agent calls it again until a verdict settles.
...
- `view_media` — View an image already in the plan. `Read` returns image markdown as raw `ref-media://` URIs, which are not fetchable directly, so pass the `mediaId` here to get a temporary link to the image. Sizes are `thumb`, `standard` (the default), and `original`. Ask for the pixels inline with `format: "image"` only when the agent cannot fetch links, since inline pixels cost vision tokens immediately, and use `original` only when the standard view is genuinely not enough, since it is full resolution.

**General mode** adds:
- `List` — List your plans
- `Create` — Create a new plan. `kind` says what the document is: `plan` (the default) for work to be reviewed and implemented, `explanation` for an explanation delivered as a link, or `receipt` for a record of work already completed. `initiative` records whose call it was to create the document — `user` when it was asked for, `agent` when the agent decided to write one.

`AwaitReview`, the `Comments` tool's `request_review` action, and `Create`'s `initiative` parameter exist only in sessions you connect yourself with your own API key — the installs on this page. Agents that Ref launches and runs for you connect as orchestrated sessions, where those three are not available at all.
