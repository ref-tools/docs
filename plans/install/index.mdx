...
Using Claude Code or Codex on your own machine? One command sets up both, signs you in through your browser, and connects their sessions to Ref Plans — see [local agents](/plans/install/local-agent). The guides below are the manual path, and the one to use for every other agent.
...
- `ListSkills` — List the available skills and their ids
- `Skill` — Load a skill by id for planning and workflow guidance
- `Comments` — Read and write comments on the plan
- `PrLinks` — Attach pull request links to the plan
- `Media` — Handle images in a plan with `action: upload | insert | view`. `upload` stores an image from a publicly fetchable HTTPS `url`, or pass `mimeType` (never both) to receive a signed `uploadUrl` for a client-side PUT. `insert` places `![alt](ref-media://mediaId)` after a line (`afterLine` is 1-indexed; `0` inserts at the top; consecutive inserts render as a grid, three per row; `displayWidth` sets the rendered width of a standalone image). `view` returns a signed URL, or inline pixels when the client cannot fetch one (`size` is thumb, standard, or original). At read level (viewer link), the tool exposes only `action: view`.
...
- `UpdatePR` — Edit the title and description of an existing pull request
...
`ReviewPR`, `UpdatePR`, and `MergePR` availability depends on your GitHub permissions settings. See the [GitHub integration page](/plans/integrations/github) for which toggle governs each tool.
...
<CardGroup cols={2}>
  <Card title="Local agents" icon="terminal" href="/plans/install/local-agent">
    Set up Claude Code and Codex with one command.
  </Card>
