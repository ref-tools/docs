...
Using Claude Code or Codex on your own machine? One command sets up both, signs you in through your browser, and installs the planning loop along with the MCP server — see the [quick install](/plans/install/local-agent). The guides below are the manual path, and the one to use for every other agent.
...
<CardGroup cols={2}>
  <Card title="Quick Install" icon="terminal" href="/plans/install/local-agent">
