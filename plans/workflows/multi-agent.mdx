...
Multi-agent orchestration lets a Ref agent (the orchestrator) spawn sub-agents on Cursor, Devin, Warp OZ, Conductor Cloud, or internal ref-threads, send them messages, and receive updates back — all via MCP tools. This is the most automated way to use Ref Plans.
...
| `conductor-cloud` | Implementation tasks | Launches a Conductor Cloud session that writes code in a Conductor workspace |
...
- **`harness`** — Where to run the agent: `cursor`, `devin`, `warp-oz`, `conductor-cloud`, or `ref-thread`
- **`taskShortDescription`** — Brief label for the agent (~10 words)
- **`repo`** / **`branch`** / **`model`** — Optional overrides for the `cursor` and `conductor-cloud` harnesses. Leave `repo` out and Cursor falls back to your configured repository, Conductor Cloud to its configured project
...
- **`fromAgentId`** — The agent sending the message. Always pass your own agent ID; it defaults to the current agent only in agent-scoped MCP sessions
...
<Steps>
  ...
  <Step title="Orchestrate">
    An orchestrator agent reads the plan and launches implementation agents (Cursor, Devin, Warp OZ, or Conductor Cloud) in parallel via `LaunchAgent`. Each agent gets a self-contained prompt with all the context it needs.
  </Step>
  <Step title="Review">
...
**Use `cursor`, `devin`, `warp-oz`, or `conductor-cloud` for implementation** — these agents write code in your repo. Cursor, Devin, and Warp OZ create branches and open PRs, and Ref picks up the PR URL as they work; Conductor Cloud works in a Conductor workspace. Choose based on which service you have configured and prefer.
...
2. **API keys configured** for Cursor, Devin, Warp OZ, and/or Conductor Cloud in [Agent Settings](https://plan.ref.tools/settings) — needed for the `cursor`, `devin`, `warp-oz`, and `conductor-cloud` harnesses
...
- Don't message an agent to acknowledge receipt of its message, and don't acknowledge an acknowledgement
- The workflow is message-driven: after launching agents, wait for messages rather than polling

### Waiting on CI

CI, deploys, and humans do not send messages — "wait for a message" only covers child agents. `ref-thread` agents with wake scheduling enabled arm a wake with the `Schedule` tool (`action: create`, `delaySeconds`), end the turn, and are woken by their own message to check again — so a reviewer that finds checks still running can merge on green without a human nudging it; without the `Schedule` tool the agent reports the pending state and stops.

- `Schedule` is a ref-thread tool: Cursor, Devin, and Warp OZ children do not have it.
